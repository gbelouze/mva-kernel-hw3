...
def plotHyperSurface(
    ...
    if model.type == "linear":
        ...
        Y30 = model.f(xy).reshape(X0.shape) + intercept
        ax.contour(
...
def plotClassification(
    ...
    colors = ["blue", "red"]
    labels = [1, -1]
    cmap = pltcolors.ListedColormap(colors)  # noqa: F841
    if ax is None:
        fig, ax = plt.subplots(1, figsize=(11, 7))
    for k, label in enumerate(labels):
        im = ax.scatter(  # noqa: F841
    ...
    if model is not None:
        ...
        if model.support is not None:
            ...
            signedDist = model.f(model.support)
            plotHyperSurface(
                ax,
                xx,
                model,
            ...
            print("Warning : [support] attribute missing.")
